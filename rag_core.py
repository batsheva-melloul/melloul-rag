...
SYSTEM_PROMPT = (
    ...
    "visual inside a fenced block tagged exactly ```infographic (never ```svg, ```html, "
    "or unfenced). Build it from ordinary HTML ELEMENTS (div, span, ul, table) styled "
    "with INLINE style attributes. Do NOT output a standalone <svg> document and do NOT "
    "use a <style> block or CSS classes — use inline <svg> only for small shapes, icons, "
    "or chart bars. Requirements: put dir=\"rtl\" on the outer <div>, keep all text in "
    "the document's language and short, and make it responsive — max-width:100% on the "
    "outer container (no fixed pixel width) and a viewBox on any <svg>. Use a clean, "
    "readable layout with clear labels and good contrast. Base every fact ONLY on the "
    "excerpts. You may add a one-line caption before the block. Never include <script> "
    "or event handlers, and do NOT produce a visual when none was requested.\n"
)
